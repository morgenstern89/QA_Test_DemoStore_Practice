from qatest.src.pages.locators.CheckoutPageLocator import CheckoutPageLocator
from qatest.src.SeleniumExtended import SeleniumExtended
from qatest.src.helpers.generic_helpers import generate_random_email_and_password
...
class CheckoutPage(CheckoutPageLocator):
    ...
    def input_billing_city_field(self,city=None):
        city='San Francisco' if not city else city
        self.sl.wait_and_input_text(self.BILLING_CITY_FIELD,city)

    def input_billing_zip_field(self, zip_code=None):
        zip_code = 94016 if not zip_code else "98500"
        self.sl.wait_and_input_text(self.BILLING_ZIP_FIELD, zip_code)

    def input_billing_phone_field(self, phone=None):
        phone = '415111111' if not phone else phone
        self.sl.wait_and_input_text(self.BILLING_PHONE_FIELD, phone)


    def input_billing_email_field(self, email=None):
        if not email:
    ...
    def fill_in_billing_info(self, f_name=None,l_name=None,street1=None, city=None,zip_code=None,phone=None,email=None):
        ...
        self.input_billing_city_field(city=city)
        self.input_billing_zip_field(zip_code=zip_code)
        self.input_billing_phone_field(phone=phone)
        self.input_billing_email_field(email=email)
